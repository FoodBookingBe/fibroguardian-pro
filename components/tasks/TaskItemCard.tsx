...
import { CalendarDays, Edit3, Eye, Trash2, Zap } from 'lucide-react'; // Zap voor Opdracht, CalendarDays voor Taak
    ...
    <li className={`p-4 rounded-lg shadow border ${cardBgColor} transition-colors duration-150`}>
      <div className="flex items-start justify-between">
        <div className="flex-grow">
          ...
          {task.status && (
            <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${task.status === 'voltooid' ? 'bg-green-200 text-green-800' : 'bg-orange-200 text-orange-800'
              }`}>
              {task.status}
            </span>
          )}
