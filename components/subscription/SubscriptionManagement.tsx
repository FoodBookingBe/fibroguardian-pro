...
if (typeof window !== 'undefined' && typeof (globalThis as any).formatCurrency !== 'function') {
  (globalThis as any).formatCurrency = (amount: number, currency = 'EUR') => {
...
const useSubscription = (userId: string | undefined) => {
  ...
  return { subscription, error, cancelSubscription, updateSubscription };
};

export function SubscriptionManagement(): JSX.Element {
  const { user } = useAuth();
  const { subscription, error, cancelSubscription, updateSubscription } = useSubscription(user?.id);
