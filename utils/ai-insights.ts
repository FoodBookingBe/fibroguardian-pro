...
export async function generateAIInsights(params: AIInsightsParams): Promise<AIInsight[]> {
  try {
    const { patientIds, insightType, timeframe: timeframeParam, limit } = params;
...
function generateSymptomPatternInsights(
  ...
  const painScores = reflections
    ...
    .map(r => ({ score: r.pijn_score, date: new Date(r.created_at || new Date()) }))
  ...
  const fatigueScores = reflections
    ...
    .map(r => ({ score: r.vermoeidheid_score, date: new Date(r.created_at || new Date()) }))
    .sort((a, b) => a.date.getTime() - b.date.getTime());

  if (fatigueScores.length >= 5) {
    // Calculate average fatigue
    const avgFatigue = fatigueScores.reduce((sum, item) => sum + (item && item.score ? item.score : 0), 0) / fatigueScores.length;
...
function generateTreatmentEfficacyInsights(
  ...
  Object.entries(tasksByType).forEach(([taskType, logs]) => {
    ...
    if (painBefore.length >= 5 && painAfter.length >= 5) {
      const avgPainBefore = painBefore.reduce((sum, score) => sum + (score ?? 0), 0) / painBefore.length;
      const avgPainAfter = painAfter.reduce((sum, score) => sum + (score ?? 0), 0) / painAfter.length;
    ...
    if (fatigueBefore.length >= 5 && fatigueAfter.length >= 5) {
      const avgFatigueBefore = fatigueBefore.reduce((sum, score) => sum + (score ?? 0), 0) / fatigueBefore.length;
      const avgFatigueAfter = fatigueAfter.reduce((sum, score) => sum + (score ?? 0), 0) / fatigueAfter.length;
...
function generateActivityCorrelationInsights(
  ...
  if (tasksWithDuration.length >= 5) {
    ...
    const tasksWithMetrics = tasksWithDuration.map(log => {
      const start = new Date(log.start_tijd || new Date());
      const end = new Date(log.eind_tijd || new Date());
    ...
    if (longTasks.length >= 3 && shortTasks.length >= 3) {
      const avgPainLong = longTasks.reduce((sum, t) => sum + (t?.painScore ?? 0), 0) / longTasks.length;
      const avgPainShort = shortTasks.reduce((sum, t) => sum + (t?.painScore ?? 0), 0) / shortTasks.length;
  ...
  const morningTasks = taskLogs.filter(log => {
    const time = new Date(log.start_tijd || new Date());
  ...
  if (morningTasks.length >= 3 && afternoonTasks.length >= 3) {
    const avgPainMorning = morningTasks
      ...
      .reduce((sum, log) => sum + (log?.pijn_score ?? 0), 0) / morningTasks.length;

    const avgPainAfternoon = afternoonTasks
      .filter(log => log?.pijn_score !== null)
      .reduce((sum, log) => sum + (log?.pijn_score ?? 0), 0) / afternoonTasks.length;
