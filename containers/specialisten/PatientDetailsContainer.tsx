...
import { useInsights, useProfile, useRecentLogs, useReflecties, useTasks } from '@/hooks/useSupabaseQuery'; // Changed useReflections to useReflecties
...
export default function PatientDetailsContainer({ patientId, initialPatientProfile }: PatientDetailsContainerProps) {
  ...
  const {
    ...
    initialData: initialPatientProfile as any,
    enabled: !!patientId,
  ...
  const { data: reflecties, isLoading: isLoadingReflections, error: reflectionsError, isError: isReflectionsError } = useReflecties(patientId, 30, { enabled: activeTab === 'reflecties' && !!patientId }); // Changed useReflections
  ...
  const metricAverages = useMemo(() => {
    ...
    const vermoeidheidScores = logs.map((log: any) => log.vermoeidheid_score).filter((s: any) => s != null) as number[];
